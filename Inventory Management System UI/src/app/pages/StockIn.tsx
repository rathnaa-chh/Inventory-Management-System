...
export default function StockIn() {
  ...
  const [recentEntries, setRecentEntries] = useState<RecentStockIn[]>([]);
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const transactionsRes = await transactionService.getAll({ type: 'IN', limit: 5 });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notifyTransactionSuccess('IN', productName, quantity);
      setSuccessMessage(`Stock in recorded successfully! Added ${quantity} units.`);
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-600 mt-2">Record incoming inventory movements.</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              placeholder="Add any notes about this stock in (e.g., Supplier name, batch number, etc.)..."
          ...
          <div className="flex gap-3 pt-6">
            ...
            >
              {submitLoading ? 'Recording...' : 'Record Stock In'}
            </Button>
          </div>
        </form>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <h3 className="text-lg font-semibold text-slate-900 mb-4">Recent Stock In Entries</h3>
            ...
            {recentEntries.map((entry) => (
              <div key={entry.id} className="flex items-center justify-between p-4 border border-slate-200 rounded-lg hover:bg-slate-50">
                <div>
                  ...
                  <p className="text-sm text-slate-500">Date: {entry.date} {entry.notes && `• ${entry.notes}`}</p>
                </div>
